        ...
        while cap.isOpened():
            ...
            if not landmarks is None:
                ...
                match train_mode:
                    ...
                    case cl.PUSH_UP:
                        image = drawing_push_up_landmarks(image, landmarks, landmarks_idx)
...
def recording_format()->Any:
    ...
    writer = cv.VideoWriter(video_out, fmt, cl.VIDEO_FRAME_RATE10, (cl.VIDEO_FRAME_WIDTH, cl.VIDEO_FRAME_HEIGHT))
...
def drawing_base_landmarks(image, landmarks, land_idx)->np.ndarray:
    ...
    cv.circle(image, (ex_l_sholder.x, ex_l_sholder.y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)
    ...
    cv.line(image, (l_sholder_x, l_sholder_y), (ex_l_sholder.x, ex_l_sholder.y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
...
def drawing_squad_landmarks(image, landmarks, land_idx)->np.ndarray:
    ...
    l_knee_x, l_knee_y = cie_landmark2videosize(landmark[land_idx.LEFT_KNEE].x, landmark[land_idx.LEFT_KNEE].y)
    # cv.circle(image, (r_knee_x, r_knee_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)
    cv.circle(image, (l_knee_x, l_knee_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)
    ...
    l_ankle_x, l_ankle_y = cie_landmark2videosize(landmark[land_idx.LEFT_ANKLE].x, landmark[land_idx.LEFT_ANKLE].y)
    # cv.circle(image, (r_ankle_x, r_ankle_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)
    cv.circle(image, (l_ankle_x, l_ankle_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)

    # drawing heel points
    # r_heel_x, r_heel_y = cie_landmark2videosize(landmark[land_idx.RIGHT_HEEL].x, landmark[land_idx.RIGHT_HEEL].y)
    l_heel_x, l_heel_y = cie_landmark2videosize(landmark[land_idx.LEFT_HEEL].x, landmark[land_idx.LEFT_HEEL].y)
    # cv.circle(image, (r_heel_x, r_heel_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)
    cv.circle(image, (l_heel_x, l_heel_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)

    # drawing toe points
    # r_toe_x, r_toe_y = cie_landmark2videosize(landmark[land_idx.RIGHT_FOOT_INDEX].x, landmark[land_idx.RIGHT_FOOT_INDEX].y)
    l_toe_x, l_toe_y = cie_landmark2videosize(landmark[land_idx.LEFT_FOOT_INDEX].x, landmark[land_idx.LEFT_FOOT_INDEX].y)
    # cv.circle(image, (r_toe_x, r_toe_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)
    cv.circle(image, (l_toe_x, l_toe_y), cl.RADIUS_SIZE7, cl.COLOR_BLUE, cl.THICKNES_SIZE3)

    # drawing line connections
    # cv.line(image, (r_waist_x, r_waist_y), (r_knee_x, r_knee_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
    cv.line(image, (l_waist_x, l_waist_y), (l_knee_x, l_knee_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
    # cv.line(image, (r_knee_x, r_knee_y), (r_ankle_x, r_ankle_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
    cv.line(image, (l_knee_x, l_knee_y), (l_ankle_x, l_ankle_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
    # cv.line(image, (r_ankle_x, r_ankle_y), (r_heel_x, r_heel_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
    cv.line(image, (l_ankle_x, l_ankle_y), (l_heel_x, l_heel_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
    # cv.line(image, (r_heel_x, r_heel_y), (r_toe_x, r_toe_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
    cv.line(image, (l_heel_x, l_heel_y), (l_toe_x, l_toe_y), cl.COLOR_GREEN, cl.THICKNES_SIZE3, cv.LINE_4, 0)
